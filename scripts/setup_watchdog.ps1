...
$c = Get-Credential -UserName $env:USERNAME -Message "Enter user credentials:"
$exePath = Join-Path -Path $outputPath -ChildPath "watchdog.exe"
$response = Invoke-WebRequest -Uri $url -OutFile $exePath -TimeoutSec 5
$taskAction = New-ScheduledTaskAction -Execute "$exePath" -Argument "-f $manifestsPath -m $completedPath"
...
$taskSettings = New-ScheduledTaskSettingsSet -DontStopOnIdleEnd -ExecutionTimeLimit '00:00:00' -Hidden
$taskPrincipal = New-ScheduledTaskPrincipal -UserId $c.UserName -LogonType Password -RunLevel Highest
$taskPath = "AIND"
...
Register-ScheduledTask -TaskName $fullTaskPath -Action $taskAction -Trigger @($taskTriggerStartup, $taskTriggerLogOn) -Settings $taskSettings -Principal $taskPrincipal -Password $c.Password -User $c.UserName
